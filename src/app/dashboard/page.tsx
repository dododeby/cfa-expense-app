...
export default function DashboardPage() {
    ...
    const groupData = accounts.reduce((acc, account) => {
        if (account.type === 'Analítica' && account.group) {
            const rowData = expenseData[account.id] || { total: 0, finalistica: 0 }
            const existing = acc.find(item => item.name === account.group)
...
function ConsolidatedDashboard() {
    ...
    const accounts: Account[] = allAccountsData as Account[]
    // Get groups from accounts for the filter
    const groups = Array.from(new Set(accounts.map(a => a.group).filter(Boolean))).sort()
    ...
    const getChartData = () => {
        ...
        let chartData = orgs.map(org => {
            ...
            // Filter accounts based on group selection
            const targetAccounts = accounts.filter(a =>
                a.type === 'Analítica' &&
                (chartGroupFilter === 'all' || a.group === chartGroupFilter)
            )
